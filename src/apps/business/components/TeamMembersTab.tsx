...
export function TeamMembersTab() {
  ...
  const handleRemoveMember = async (memberId: string) => {
    ...
    try {
      await supabase.from('company_users').update({ is_active: false }).eq('id', memberId);
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <CardDescription>Todos os colaboradores vinculados à sua empresa</CardDescription>
            </div>
