...
const generateSimulatedMapContent = (journeyResults: any[], lang: string): MapSection[] => {
  ...
  const getDominantTraits = () => {
    ...
    journeyResults.forEach(result => {
      ...
      if ((result.testType === 'estilos_conexao_afetiva' || result.testType === 'linguagens_amor') && result.primary) {
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-muted/30 rounded-xl p-4 text-center">
              <p className="text-xs text-muted-foreground">
                {isEn 
                  ? "This is a simulated preview. In production, the map would be generated by Nello AI based on the user's actual test results."
                  : "Esta é uma pré-visualização simulada. Em produção, o mapa seria gerado pela IA Nello baseado nos resultados reais dos testes do usuário."
                }
              </p>
            </div>
          </div>
