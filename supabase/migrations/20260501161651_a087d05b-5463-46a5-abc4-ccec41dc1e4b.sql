...
AS $$
  ...
  base AS (
    -- 1) quem deu consentimento individual no Discernir
    SELECT
      c.user_id,
      MIN(c.granted_at) AS registered_at
    FROM public.discernir_consents c
    WHERE c.consent_type = 'individual'
      AND c.is_active = true
    GROUP BY c.user_id
    UNION
    -- 2) quem tem qualquer perfil (mesmo sem consentimento gravado)
    SELECT
      cp.user_id,
      MIN(cp.created_at) AS registered_at
  ...
  agg AS (
    SELECT
      b.user_id,
      MIN(b.registered_at) AS registered_at
    FROM base b
    GROUP BY b.user_id
  )
  SELECT
